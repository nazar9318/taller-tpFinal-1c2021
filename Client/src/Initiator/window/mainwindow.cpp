...
MainWindow::MainWindow(Socket& skt, bool &started,
    ...
    connect(this->ui->OK, SIGNAL(pressed()), this, SLOT(on_OK_clicked()));
...
void MainWindow::on_OK_clicked() {
    user_name = this->ui->lineEdit->displayText().toStdString();
    if (user_name.size() < MIN_NAME_LENGTH) {
...
void MainWindow::createMatch(const QString& map_name) {
    bool match_name_inserted = false;
    QLineEdit::EchoMode echo = QLineEdit::Normal;
    QString qname = QInputDialog::getText(this, "entrada", "Ingrese un"
            " nombre para la partida", echo, "", &match_name_inserted, NULL, NULL);
    if (!match_name_inserted) { return; }
